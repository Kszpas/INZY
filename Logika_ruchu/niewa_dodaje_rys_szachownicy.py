...
CAMERA_INDEX = 0
OUTPUT_SIZE = (512, 512)
ANALYSIS_INTERVAL = 5.0  # sekundy między analizami
...
def analyze_board_with_model(warped_img, v_lines, h_lines):
    ...
    # DZIELENIE ODJĘTE Z BŁĘDNEGO ODBICIA — TERAZ PRAWIDŁOWE KOLEJNOŚCI (od lewej do prawej)
    for r in range(8):
        ...
        if candidate_count >= REQUIRED_CONSECUTIVE:
            ...
            with lock:
                ...
                if applied:
                    ...
                    log_line = f"{timestamp}  REJECTED {desc}"
                    move_history.append(log_line)
                    save_move_to_file(log_line)
...
while True:
    ...
    with lock:
        ...
        # narysuj historię ruchów na kopii (jeżeli są jakieś wpisy)
        if move_history:
