...
def analyze_board_with_model(warped_img, v_lines, h_lines):
    """Dzielenie szachownicy na 64 pola, zapis do JPG (odwrócone lewo‑prawo)
       oraz predykcja dla całej planszy."""
    ...
    # --- DZIELENIE POLA PO ODBICIU LEWO‑PRAWO ---
    for r in range(8):
        ...
        for c in range(8): # od lewej do prawej
            ...
            # nazwy pól szachowych (odwrócone lewo‑prawo, a1 = prawy dół)
            file_char = chr(ord('a') + c)
            rank = 8 - r            # gora obrazu → rzad 8, dol → rzad 1
            sq = f"{file_char}{rank}"

            tiles.append(tile)
            coords.append(sq)

    print(f"📷 Wyodrębniono {len(tiles)} pól szachownicy (odwrócenie lewo‑prawo).")

    # --- ZAPIS WSZYSTKICH POL DO JPG ---
    ...
    print("💾 Zapisano 64 pola w folderze 'all_tiles' (odwrócenie lewo‑prawo).")

    # --- PREDYKCJA DLA CAŁEJ PLANSZY ---
    preds = model.predict(np.stack(tiles), verbose=0)
    labels = [CLASS_NAMES[np.argmax(p)] for p in preds]
    board_results = list(zip(coords, labels))

    # wydruk w czytelnej formie po rzędach 8→1
    print("📊 Predykcja dla całej planszy (od czarnych → do białych):")
    for rank in range(8, 0, -1):             # 8,7,…,1
        rank_data = [(sq, lbl) for sq, lbl in board_results if sq.endswith(str(rank))]
        print(f"Rząd {rank}: ", rank_data)

    return dict(board_results)
